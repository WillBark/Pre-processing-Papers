...
#Read in Data
data <- read.table('GSE207209_sample_id_ReadsperGene.txt', header = TRUE, sep = '\t')
head(data)
dim(data)

#Get Metadata
gse <- getGEO(GEO = 'GSE207209', GSEMatrix = TRUE)
gse
...
metadata_modified  <-  metadata %>%
  select(1, 11, 12) %>% #Select required columns
  #Rename columsn for ease of use
  rename(ID = title) %>%
  rename(tissue = characteristics_ch1.1) %>%
  rename(race = characteristics_ch1.2) %>%
  #Remove prefixes
  mutate(tissue = gsub('tissue type: ', '', tissue)) %>%
...
#Clean sample names to match metadata IDs
data_long$samples <- gsub("Myo_", "MyoF_", data_long$samples)
colnames(data)[2:ncol(data)] <- gsub("Myo_", "MyoF_", colnames(data)[2:ncol(data)])
...
rownames(matrix_data)<-data$gene_id
...
#prepping metadata and data for PCA -> run the code CBF_PCA_Paper4.R
...
gsub(pattern = "Myometrium","Myometrium",metadata_modified_sel$homoGroup)  
gsub(pattern = "Fibroid","Fibroid",metadata_modified_sel$homoGroup)
...
write.csv(log2_normalised_data, 'log2_paper4', row.names = TRUE)
