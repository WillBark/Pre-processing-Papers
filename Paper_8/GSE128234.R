...
#Read in Data
data <- read.csv("matrix_paper8.csv", header = TRUE, sep = ",")
head(data)
dim(data)

data[is.na(data)] <- 0 #Replacing NA with 0 so can be used in analysis
names(data)[1] <- 'GeneID'

#Get Metadata
gse <- getGEO(GEO = 'GSE128234', GSEMatrix = TRUE)
gse
...
metadata_modified  <-  metadata %>%
  select(1, 8, 10) %>% #Selecting required columns
...
#So that metadata IDs match column names in data need to remove prefix
metadata_modified$ID <- gsub(" \\[capture Hi-C\\]", "", metadata_modified$ID)
metadata_modified$ID <- gsub("-", "_", metadata_modified$ID)

#Reshaping data from wide to long
data_long  <-  data %>%
  gather(key = 'samples', value = 'FPKM', -GeneID)

#Joining data_long and metadata_modified on samples and ID columns
data_long <- data_long %>%
...
#prepping metadata and data for PCA -> run the code CBF_PCA_Paper8.R
...
write.csv(log2_normalised_data, 'log2_paper8', row.names = TRUE)
