...
#Read in Data
data <- read.csv('GSE234528_data.csv', header = TRUE, sep = ',')
head(data)
dim(data)

#Get Metadata
gse <- getGEO(GEO = 'GSE234528', GSEMatrix = TRUE)
gse
...
metadata_modified  <-  metadata %>%
  select(1, 2, 10, 11, 12, 15) %>% #Select columns needed for metadata
  #Rename columns for better understanding
  rename(ID = geo_accession) %>%
  rename(tissue = characteristics_ch1) %>%
  rename(cell_type = characteristics_ch1.1) %>%
  rename(individual = characteristics_ch1.2) %>%
  rename(age = characteristics_ch1.5) %>%
  #Remove prefixes
  mutate(tissue = gsub('tissue: ', '', tissue)) %>%
  mutate(cell_type = gsub('cell type: ', '', cell_type)) %>%
  mutate(age = gsub('age: ', '', age)) %>%
  mutate(individual = gsub('individual: ', '', individual))
...
#prepping metadata and data for PCA -> run the code CBF_PCA_Paper2.R
...
#Saving data as log2 data frame
log2_normalised_data <- as.data.frame(log2_normalized_matrix)
write.csv(log2_normalised_data, 'log2_paper2', row.names = TRUE)
