...
#Read in Data
data <- read.table("GSE169255_sample_id_ReadsperGene.txt", header = TRUE, sep = "\t")
head(data)
dim(data)

#Get Metadata
gse <- getGEO(GEO = 'GSE169255', GSEMatrix = TRUE)
gse
...
metadata_modified  <-  metadata %>%
  ...
  rename(ID = title) %>%
  rename(diagnosis = characteristics_ch1) %>%
...
#Joining data_long and metadata_modified through samples and ID columns
data_long <- data_long %>%
...
#prepping metadata and data for PCA -> run the code CBF_PCA_Paper7.R
...
write.csv(log2_normalised_data, 'log2_paper7', row.names = TRUE)
